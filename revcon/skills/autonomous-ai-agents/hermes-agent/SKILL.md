...
hermes mcp add NAME         Add an MCP server (--url or --command+--args)
...
Config file key is **top-level** `mcp_servers:`, not `mcp.servers:`. The
built-in MCP client connects servers at startup (`mcp` package required),
discovers tools, and exposes them as `mcp_<server>_<tool>`.
Full MCP client details, stdio/HTTP examples, Windows stdio workarounds, auth,
and troubleshooting: `skill_view(name="hermes-agent", file_path="references/native-mcp.md")`.
